...
switch($_GET[act]){
  ...
  echo "<div style='text-align:right;padding-right:30px;'>
    ...
    echo"<datalist id='auto'>";
     $qry=mysqli_query($GLOBALS["___mysqli_ston"], "SELECT * FROM tbl_soal");
  ...
  //Tampil Data Soal    
    ...
    $tampil=mysqli_query($GLOBALS["___mysqli_ston"], "SELECT * FROM tbl_soal ORDER BY id_soal DESC");
    $no=1;
  ...
  // Form Edit Soal  
  case "editsoal":
    $edit=mysqli_query($GLOBALS["___mysqli_ston"], "SELECT * FROM tbl_soal WHERE id_soal='$_GET[id]'");
  ...
  case "carisoal":
    ...
    $tampil=mysqli_query($GLOBALS["___mysqli_ston"], "SELECT * FROM tbl_soal WHERE soal LIKE '%$_POST[cari]%'");
    $no=1;
    while ($r=mysqli_fetch_array($tampil)){
       echo "<tr><td>$no</td>
             <td>$r[soal]</td>
